...
Goアプリは`playbooks/go-sample/go-app`ディレクトリを単独プロジェクトとして管理し、playbook実行時にサーバへ配備されます。
...
- Goアプリ本体は `playbooks/go-sample/go-app` ディレクトリで管理します
- デプロイ時は `playbooks/go-sample/main.yml` が `playbooks/go-sample/go-app` を `/opt/go-sample-app/go-app` へコピーしてビルドします
